...
def add_exerciseid_in_structure(wb_s,cursor,row,cnxn, structure_file):
    print(row)
    ...
    if all(x is None for x in entry):
        ...
        if None in entry:
            ...
            if not list:
                sqlcommand_insert = 'INSERT INTO [dbo].[WrapperExercises] VALUES('
                sqlcommand = sqlcommand_insert
                ...
                for id in columns[names.index('WrapperExercises')]:
                    ...
                    count = count + 1
                sqlcommand= sqlcommand + ')'
                print(sqlcommand, entry)
                
                
                
                if sqlcommand == sqlcommand_insert:
                    pass
                else:
...
def main(course_folder,cursor, cnxn):
    ...
    if structure_file.exists():
        ...
        action_col = get_column(sheet=sheet, row = 2, name='Exercise_Id')
        ...
        for line in to_submit:
            print(line)
            add_exerciseid_in_structure(wb_s=wb_structure,cursor=cursor,row=line,cnxn=cnxn, structure_file=structure_file)
