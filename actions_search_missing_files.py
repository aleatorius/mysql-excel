...
def main(folder):
    ...
    if structure_path.exists():
        ...
        for folder in to_diff[3:]:
            ...
            output,header_cells = missing_in_excel(excel=excel)
            print(header_cells)
            ...
            if output:
                if firstrun == True:
                    ...
                    sheet_wbout.title = "Missing sound files"
                    header = ['Exercise']+output[0]
                    sheet_wbout.append(header)
                    line = [str(excel)]+(len(header)-1)*['']
                    sheet_wbout.append(line)
                    ...
                    for line in output[1:]:
                        sheet_wbout.append(['']+line)
                nonzero_header_cells = [1]+header_cells
        get_style(header_cells=nonzero_header_cells,sheet=sheet_wbout)    
        wbout.save('missing.xlsx')
